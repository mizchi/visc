...
import { SnapshotComparator } from "./snapshot-comparator.ts";
import { VisualCheckConfig, TestResult } from "./types.ts";
  ...
  async runTests(updateBaseline = false): Promise<TestResult[]> {
    ...
    for (const urlConfig of this.config.urls) {
      const urlSpinner = ora(
        `Testing ${chalk.cyan(urlConfig.name)} (${urlConfig.url})`
      ).start();
      const startTime = Date.now();

      try {
        // スクリーンショットを撮影
        const currentImagePath = await this.browserController.captureScreenshot(
          urlConfig
        );
        ...
        if (updateBaseline) {
          ...
          urlSpinner.succeed(
            `${chalk.cyan(urlConfig.name)} - Baseline updated`
          );
        } else {
          // スナップショットを比較
          ...
          if (result.passed) {
            urlSpinner.succeed(
              `${chalk.cyan(urlConfig.name)} - ${chalk.green("PASSED")} (${
                result.duration
              }ms)`
            );
          } else {
            urlSpinner.fail(
              `${chalk.cyan(urlConfig.name)} - ${chalk.red("FAILED")} (${
                result.duration
              }ms)`
            );
            if (result.error) {
            ...
            if (result.diffImagePath) {
              console.log(
                chalk.yellow(`  → Diff image: ${result.diffImagePath}`)
              );
            }
          }
        }
        ...
        urlSpinner.fail(
          `${chalk.cyan(urlConfig.name)} - ${chalk.red(
            "ERROR"
          )} (${duration}ms)`
        );
