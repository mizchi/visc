...
export async function compareLayoutsWithContentExclusion(
  ...
  // まず通常のレイアウトを抽出して比較
  const baselineLayout = await page1.evaluate(extractLayoutScript);
  const currentLayout = await page2.evaluate(extractLayoutScript);
  ...
  // 本文除外後のレイアウトを再抽出
  const excludedBaselineLayout = await page1.evaluate(extractLayoutScript);
  const excludedCurrentLayout = await page2.evaluate(extractLayoutScript);
...
export async function analyzeLayoutWithContentAwareness(
  ...
  // 通常のレイアウト分析
  const normalLayout = await page.evaluate(extractLayoutScript);
  ...
  // 本文除外後のレイアウト分析
  const layoutWithoutContent = await page.evaluate(extractLayoutScript);
