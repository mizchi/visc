  ...
  it("should calibrate comparison settings based on multiple samples", async () => {
    ...
    try {
      ...
      for (let i = 0; i < minSamples; i++) {
        ...
        console.log(
          `  ✓ Sample ${i + 1}: ${rawData.elements.length} raw elements → ${
            layout.visualNodeGroups?.length || 0
          } groups`
        );
      ...
      console.log(
        `  - Overall flakiness: ${initialFlakiness.overallScore.toFixed(1)}%`
      );
      console.log(
        `  - Flaky elements: ${initialFlakiness.flakyElements.length}`
      );
      ...
      console.log("\n⚙️  Initial settings:");
      console.log(
        `  - Position tolerance: ${currentSettings.settings.positionTolerance}px`
      );
      console.log(
        `  - Size tolerance: ${currentSettings.settings.sizeTolerance}%`
      );
      console.log(
        `  - Text similarity: ${(
          currentSettings.settings.textSimilarityThreshold * 100
        ).toFixed(0)}%`
      );
      ...
      while (iteration < maxIterations - minSamples) {
        ...
        console.log(
          `  📊 Added sample ${rawDataSamples.length}: ${newRawData.elements.length} raw elements`
        );
        console.log(
          `  🔄 Regenerated ${layoutSamples.length} layouts with current thresholds`
        );
        ...
        for (let i = 0; i < layoutSamples.length - 1; i++) {
          for (let j = i + 1; j < layoutSamples.length; j++) {
            const comparison = compareLayoutTrees(
              layoutSamples[i],
              layoutSamples[j],
              {
        ...
        console.log(
          `  📈 Average differences per pair: ${avgDifferences.toFixed(1)}`
        );

        // 新しいフレーキーネス分析
        const currentFlakiness = detectFlakiness(layoutSamples);
        console.log(
          `  🎯 Current flakiness: ${currentFlakiness.overallScore.toFixed(1)}%`
        );
        ...
        if (Math.abs(improvement) < 0.5) {
          convergenceCount++;
          console.log(
            `  ⚠️  Minimal improvement (${convergenceCount}/${convergenceThreshold})`
          );
        } else {
          convergenceCount = 0;
        }
        ...
        console.log(
          `     - Position: ${currentSettings.settings.positionTolerance}px`
        );
      ...
      console.log(
        `  - Importance threshold: ${currentSettings.settings.importanceThreshold}`
      );
      console.log(
        `  - Grouping threshold: ${currentSettings.settings.importanceThreshold}`
      );
      ...
      expect(currentSettings.settings.textSimilarityThreshold).toBeGreaterThan(
        0
      );
      expect(
        currentSettings.settings.textSimilarityThreshold
      ...
      if (lastFlakinessScore > 0) {
        ...
        finalFlakiness.flakyElements.slice(0, 5).forEach((element, i) => {
          ...
          console.log(
            `     - Flakiness: ${element.score.toFixed(1)}%`
          );
