...
function compareSemanticGroupChildren(
  ...
  if (baselineGroup.children && currentGroup.children) {
    // 子要素の削除を検出
    baselineGroup.children.forEach((child: LayoutElement | SemanticGroup, index: number) => {
      const found = currentGroup.children.find((c: LayoutElement | SemanticGroup) => isSameElement(child, c));
      if (!found) {
        differences.push({
          type: "removed",
    ...
    // 子要素の追加・変更を検出
    currentGroup.children.forEach((child: LayoutElement | SemanticGroup, index: number) => {
      const found = baselineGroup.children.find((c: LayoutElement | SemanticGroup) => isSameElement(child, c));
      if (!found) {
        differences.push({
          type: "added",
          path: `${parentPath}/child[${index}]`,
...
export function compareLayouts(
  ...
  if (baseline.elements && current.elements) {
    ...
    // 要素をマッチング
    currentElements.forEach((currentEl: LayoutElement, currentIndex: number) => {
      let bestMatch: { element: LayoutElement; index: number } | null = null;
      let bestSimilarity = 0;
      
      baselineElements.forEach((baselineEl: LayoutElement, baselineIndex: number) => {
    ...
    // 削除された要素を検出
    baselineElements.forEach((element: LayoutElement, index: number) => {
    ...
    // 追加された要素を検出
    currentElements.forEach((element: LayoutElement, index: number) => {
  ...
  if (baseline.semanticGroups && current.semanticGroups) {
    ...
    // まず同じ要素をマッチング
    currentGroups.forEach((currentGroup: SemanticGroup, currentIndex: number) => {
      let bestMatch: MatchedGroup | null = null;
      let bestSimilarity = 0;

      baselineGroups.forEach((baselineGroup: SemanticGroup, baselineIndex: number) => {
    ...
    // 削除された要素を検出
    baselineGroups.forEach((group: SemanticGroup, index: number) => {
    ...
    // 追加された要素を検出
    currentGroups.forEach((group: SemanticGroup, index: number) => {
