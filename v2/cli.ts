...
program
  ...
  .option('-o, --output <path>', 'Output path for settings file', './visual-checker-settings.json')
  ...
  .action(async (url, options) => {
    ...
    try {
      ...
      spinner.succeed(chalk.green(`Calibration complete! Settings saved to ${options.output}`));
      ...
      console.log(`Text Similarity: ${Math.round(calibrationResult.settings.textSimilarityThreshold * 100)}%`);
...
program
  ...
  .option('-s, --settings <path>', 'Path to settings file', './visual-checker-settings.json')
  .option('-b, --baseline <path>', 'Path to baseline layout file (optional)')
  .option('-o, --output <dir>', 'Output directory for results', './visual-checker-output')
  ...
  .action(async (url, options) => {
    ...
    try {
      ...
      if (options.baseline) {
        ...
        await fs.writeFile(baselinePath, JSON.stringify(baselineLayout, null, 2));
        spinner.info(chalk.yellow(`No baseline provided. Saved current layout as baseline: ${baselinePath}`));
        process.exit(0);
      }
      ...
      const resultPath = path.join(options.output, `validation-${timestamp}.json`);
      ...
      if (options.saveSvg) {
        ...
        // 比較結果のSVG
        const comparison = compareLayouts(baselineLayout, currentLayout);
      ...
      console.log(`Changed Elements: ${validationResult.summary.changedElements}`);
      ...
      if (validationResult.violations.length > 0) {
        ...
        validationResult.violations
          ...
          .forEach(v => {
            ...
            console.log(`${icon} ${v.elementId}: ${v.type} (expected: ${v.expected}, actual: ${Math.round(v.actual)})`);
          });
        
        if (validationResult.violations.length > 10) {
          console.log(chalk.gray(`... and ${validationResult.violations.length - 10} more violations`));
        }
      }
...
program
  ...
  .action(async (source1, source2, options) => {
    ...
    try {
      ...
      await fs.writeFile(path.join(options.output, `layout1-${timestamp}.svg`), svg1);
      await fs.writeFile(path.join(options.output, `layout2-${timestamp}.svg`), svg2);
  ...
  .action(async (url, options) => {
    ...
    try {
      ...
      if (options.svg) {
        ...
        spinner.succeed(chalk.green(`Layout extracted to ${options.output} and ${svgPath}`));
      } else {
